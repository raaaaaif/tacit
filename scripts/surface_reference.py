...
def build(v,tilt,contact,n):
    ...
    for k in range(1,5):
        ...
        for j in range(n):
            ...
            ring.append(vertex(r*math.cos(a),r*math.sin(a),zz,True,k==4))
        rings.append(ring)
    ...
    out=[f'// TACIT v={v} uL tilt={tilt} contact={contact} n={n}',f'gravity_constant {g:.12g}','define vertex attribute surface integer',f'parameter wall_tension = {-math.cos(math.radians(contact)):.12g}','constraint 1',f'formula: {localx}^2+y^2-({rad})^2=0','vertices']
    for i,p in enumerate(vertices,1):out.append(f'{i} '+' '.join(f'{x:.12g}' for x in p)+(' fixed' if i in fixed else '')+(' constraint 1' if i in wall else '')+f' surface {1 if i in surface else 0}')
    out.append('edges')
...
def main():
    ...
    cases=[]
    combinations=[(850,0,70)] if args.smoke else [(v,t,a) for v in [100,200,400,850] for t in [0,5,10] for a in [40,70,100]]
    for v,t,a in combinations:
        pair=[solve(args.binary,v,t,a,n) for n in [24,48]]
