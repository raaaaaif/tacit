...
import type { ObservationPacket } from "../model/types";
export function CameraFrame({
  ...
  useEffect(() => {
    ...
    canvas.current.height = packet.calibration.height;
    ctx.putImageData(
      new ImageData(
        new Uint8ClampedArray(packet.pixels),
  ...
  return (
    <div className={`camera-frame ${active ? "camera-active" : ""}`}>
      <div className="camera-title">
        ...
        {label}
        <span className="mono">{packet ? "RGB" : "—"}</span>
      </div>
